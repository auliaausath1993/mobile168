...
<!-- Content
	...
	<section id="content" >
				...
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header">
							Menu <small> Untuk restok stok</small>
						</h1>
						<h4>Kontrol Stok</h4>
						<ol class="breadcrumb">
							<li class="active">
					...
					<div class="col-lg-12">
						...
						<div class="row search-stock-wrapper">
							...
							<div class="search-name-wrapper col-sm-4">
								<form name="form1" id="form1" method="post" action="<?=base_url()?>administrator/main/search_product_session_restok" >
						...
						<!-- <form onsubmit="return confirm('Update Re-Stok ?');" action="<?= base_url('administrator/main/stock_update_restok') ?>" method="post"> -->
							...
							<table id="table-exsport" class="table table-bordered table-striped">
								<thead>
									...
									<tr>
										...
										<th width="14%">Stok Sekarang</th>
										<th width="10%">Re-Stok</th>
										<!-- <th>Jangan tes</th> -->
									</tr>
								</thead>
								<tbody>
							...
							// SUM
										...
										<tr>
											...
											<td class="td-stock"><?=$items->stock ?></td>
											<td class="td-id" style="display: none;"><?=$items->id ?></td>
											<td>
												<input type="hidden" name="item_id[]"  value="<?=$items->id ?>"/>
												<input type="number" placeholder="Tambah Stok" name="stock[]" class="form-control stock-control input-small" value="" />
												<span style="display: none;"><?=$items->stock ?></span>
											</td>
											<!-- <td>
												<button type="button" class="btn btn-primary submit-modal" data-toggle="modal" data-target="#exampleModal<?=$items->id?>">
													Modal detail
												</button>
											</td> -->
									</tr>
        ...
        <!-- Modal -->
        <?php $no=0; foreach($list_item->result() as $modals) :  ?>
        <div class="modal fade" id="exampleModal<?=$modals->id?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        	<div class="modal-dialog" role="document">
        		<div class="modal-content">
        			...
        			<input type="hidden" name="item_id[]"  value="<?=$modals->id ?>"/>
        			<div class="modal-body">
        				Detail <?=$modals->variant?>
        				<br>
    ...
    <div class="modal fade" id="alert-modal" tabindex="-1" role="dialog" aria-labelledby="alertModal" aria-hidden="true">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<form action="<?= base_url('administrator/main/stock_update_restok') ?>" method="post">
						<div class="modal-header">
							<h3 class="text-center modal-title">Stock yang Akan diRe-Stok</h3>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
						</div>
