...
<?php foreach($list as $num_row => $row){ ?>
    <tr>
        <td class="hidden" <?php if ($unset_delete) { ?> style="border-right: none;"<?php } ?>
        ...
        <td <?php if ($unset_delete) { ?> style="border-left: none;"<?php } ?>
            <?php if ($buttons_counter === 0) {?>class="hidden"<?php }?>> <center>
                <div class="only-desktops"  style="white-space: normal;">
                    <?php if(!$unset_edit){?>
                        <a class="btn btn-info" href="<?php echo $row->edit_url?>"><i class="fa fa-pencil"></i> <?php echo $this->l('list_edit'); ?></a>
                    <?php } ?>
                    <?php if (!empty($row->action_urls) || !$unset_read || !$unset_delete) { ?>

                        <?php if ($show_more_button) { ?>
                            <div class="btn-group">
                                <button type="button" class="btn btn-danger dropdown-toggle gc-bootstrap-dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <?php if (!$unset_delete) { ?>
                                        <li>
                                            <a data-target="<?php echo $row->delete_url?>" href="javascript:void(0)" title="<?php echo $this->l('list_delete')?>" class="btn-sm delete-row">
                            ...
                            <?php } else {
                                if(!empty($row->action_urls)){
                                    foreach($row->action_urls as $action_unique_id => $action_url){
                                        $action = $actions[$action_unique_id];
                                        ?>
                                        <a href="<?php echo $action_url; ?>" class="m-b-mini btn-sm btn btn-default <?php echo $action->css_class; ?>">
                                            <i class="fa"></i> <?php echo $action->label?>
                                        </a>
                                    <?php }
                                }
                                ...
                                if (!$unset_delete) { ?>
                                    <a data-target="<?php echo $row->delete_url?>" href="javascript:void(0)" title="<?php echo $this->l('list_delete')?>" class="btn-sm delete-row btn btn-default">
                                        <i class="fa fa-trash-o text-danger"></i>
                                        <span class="text-danger"><?php echo $this->l('list_delete')?></span>
                                    </a>
                                <?php } ?>
                        ...
                        <ul class="dropdown-menu">
                            ...
                            if(!empty($row->action_urls)){
                                foreach($row->action_urls as $action_unique_id => $action_url){
                                    ...
                                    ?>
                                    <li>
                                        <a href="<?php echo $action_url; ?>" class="btn-sm <?php echo $action->css_class; ?>">
                                            <i class="fa"></i> <?php echo $action->label?>
                                        </a>
                                    </li>
                                <?php }
